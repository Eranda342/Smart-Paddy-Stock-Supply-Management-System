...
export default function FarmerNegotiations() {
  ...
  const fetchNegotiations = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Farmers see negotiations where they are the farmer
  ...
  const userGroups = useMemo(() => {
    ...
    allNegotiations.forEach(n => {
      const millId = n.millOwner?._id || n.millOwner;
      if (!millId) return;
      if (!map.has(millId)) {
        map.set(millId, {
          user: n.millOwner, userId: millId,
          latestUpdatedAt: new Date(n.updatedAt),
          unreadCount: n.unreadCountFarmer || 0,
          listings: [n],
        });
      } else {
        const g = map.get(millId);
        g.listings.push(n);
        if (new Date(n.updatedAt) > g.latestUpdatedAt) g.latestUpdatedAt = new Date(n.updatedAt);
        g.unreadCount += (n.unreadCountFarmer || 0);
      }
    });
  ...
  // Auto-select first
  useEffect(() => {
    if (!selectedUserId && userGroups.length > 0) setSelectedUserId(userGroups[0].userId);
  }, [userGroups]);
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      <div className="mb-8">
        <h1 className="text-xl sm:text-3xl font-bold mb-1">My Negotiations</h1>
        <p className="text-muted-foreground text-sm">
          Track and respond to offers from mill owners across your paddy listings.
        </p>
      </div>

      {/* ── Layout container ─────────────────────────────────────────────────── */}
      {/*  Mobile: overflow-hidden clip + translate-x slide                      */}
      {/*  Desktop (md+): standard 2-pane grid, no clipping                      */}
      <div className="relative overflow-hidden h-[calc(100dvh-200px)] md:h-[calc(100dvh-200px)] md:overflow-visible">
        <div
          className={`
            flex h-full transition-transform duration-300
            md:grid md:grid-cols-[320px_1fr] md:gap-5 md:transform-none
            ${mobileView === "chat" ? "-translate-x-full" : "translate-x-0"}
          `}
        >

          {/* ── Sidebar ───────────────────────────────────────────────────────── */}
          <div className="w-full min-w-full shrink-0 md:min-w-0 h-full">
            <div className="bg-card border border-border/60 rounded-2xl flex flex-col overflow-hidden shadow-md h-full">
              ...
              <div className="px-4 py-3.5 border-b border-border/60 bg-[#0f1319] shrink-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] bg-white/5 text-muted-foreground px-2 py-0.5 rounded-full border border-border/40">
                    {userGroups.length} mill{userGroups.length !== 1 ? "s" : ""}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-48 text-center px-6">
                    ...
                    <p className="text-xs text-muted-foreground/60 mt-1">Mill owners will appear here as they respond to your listings</p>
                  </div>
                ) : (
                  userGroups.map(grp => (
                    <ConversationItem
                      ...
                      onClick={() => { setSelectedUserId(grp.userId); setMobileView("chat"); }}
                      role="farmer"
                    />
                  ))
                )}
          ...
          {/* ── Chat Panel ────────────────────────────────────────────────────── */}
          <div className="w-full min-w-full shrink-0 md:min-w-0 h-full">
            <div className="bg-card border border-border/60 rounded-2xl flex flex-col overflow-hidden shadow-md h-full min-h-0">
                  ...
                  <div className="px-5 py-4 border-b border-border/60 bg-[#0f1319] shrink-0 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="md:hidden p-2 -ml-2 mr-1 hover:bg-white/10 rounded-lg text-muted-foreground"
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold line-clamp-2 break-words">
                          {selectedGroup.user?.businessDetails?.businessName || selectedGroup.user?.fullName || selectedGroup.user?.email || "Mill Owner"}
                        </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center px-8 gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground/60 mt-1">
                      {loading ? "Loading…" : "Choose a mill owner from the sidebar"}
                    </p>
                  </div>
                </div>
              )}
