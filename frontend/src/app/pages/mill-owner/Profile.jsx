...
import {
  CheckCircle2, AlertCircle, Loader2, Phone, Mail, Camera, User, Building2, MapPin,
...
export default function MillOwnerProfile() {
  ...
  const handleResubmit = async () => {
    ...
    try {
      ...
      if (user.businessDetails?.millLocation) formData.append("millLocation", user.businessDetails.millLocation);
  ...
  const getProfileCompletion = () => {
    ...
    if (!user.businessDetails?.businessName?.trim()) missing.push("Add your business name");
    if (!user.businessDetails?.businessRegistrationNumber?.trim()) missing.push("Add business registration number");
    if (!user.businessDetails?.millLocation?.trim()) missing.push("Add mill location");
    if (!user.businessDetails?.businessDocument) missing.push("Upload business document");
  ...
  const vStatus = user?.businessDetails?.verificationStatus || "PENDING";
    ...
    <div className="max-w-[1000px] mx-auto pb-12 space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight mb-1 text-white flex items-center justify-center md:justify-start gap-3">
                {user?.businessDetails?.businessName || user?.fullName || "Mill Owner"}
              </h1>
              <p className="text-sm text-muted-foreground mt-1 mb-2 text-center md:text-left">
                Owner: {user?.fullName || "Owner"}
              </p>
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 text-sm mt-3">
                <span className="px-3 py-1 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-full font-medium">
                  Mill Owner
                </span>
                {user?.emailVerified && (
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* 3. EDIT MODE - BUSINESS DETAILS */}
          <div className="bg-card border border-border rounded-3xl p-6 sm:p-8">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <Building2 className="w-5 h-5 text-[#22C55E]" /> Business Information
            </h2>
            <div className="grid sm:grid-cols-2 gap-6">
              <div className="sm:col-span-2">
                <FormInput label="Business Name" name="businessName" value={user?.businessDetails?.businessName || ""} onChange={handleBusinessChange} disabled={!isEditing} placeholder="e.g. Silva Rice Mills" />
              </div>
              <FormInput label="Registration Number" value={user?.businessDetails?.businessRegistrationNumber || ""} disabled title="Assigned during registration" />
              <FormInput label="Mill Location" name="millLocation" icon={<MapPin className="w-3.5 h-3.5" />} value={user?.businessDetails?.millLocation || ""} onChange={handleBusinessChange} disabled={!isEditing} placeholder="e.g. Colombo" />
              <FormInput label="Mill Capacity (tonnes/day)" name="millCapacity" type="number" min="0" value={user?.businessDetails?.millCapacity || ""} onChange={handleBusinessChange} disabled={!isEditing} placeholder="e.g. 50" />
              <FormInput label="Business Phone" name="businessPhone" icon={<Phone className="w-3.5 h-3.5" />} value={user?.businessDetails?.businessPhone || ""} onChange={handleBusinessChange} disabled={!isEditing} placeholder="+94112345678" error={user?.businessDetails?.businessPhone && !PHONE_REGEX.test(user.businessDetails?.businessPhone) ? "Invalid Format" : ""} />
            </div>
          ...
          <div className="bg-card border border-border rounded-3xl p-6 sm:p-8">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/70 mt-1">
                    {vStatus === "APPROVED" && "Your business is fully verified and active."}
            ...
            {/* Document Handling */}
            <div>
              <h3 className="text-sm font-semibold text-white/60 mb-3 uppercase tracking-wider">Submitted Business Document</h3>
              ...
              <div className="flex justify-between items-center bg-black/20 border border-white/5 p-4 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-white max-w-[150px] sm:max-w-[300px] truncate">
                      {user?.businessDetails?.businessDocument || "No document uploaded"}
                    </p>
                ...
                <div className="flex gap-2 shrink-0">
                  {user?.businessDetails?.businessDocument && (
                    <Button 
                      variant="secondary" 
                      size="sm" 
              ...
              {user?.businessDetails?.businessDocument && (
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-zinc-900/50 p-3 rounded-lg border border-white/5">
                    ...
                    <time className="text-xs text-white/50">Business document uploaded.</time>
                  </div>
                </div>
              )}
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white/10 bg-zinc-900 text-purple-400 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
